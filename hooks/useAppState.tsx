...
export const multiplayerOpponentNameAtom = atom<string>('');
export const multiplayerPlayerEloAtom = atom<number>(0);
...
export function useAppState(): [AppState, SetAppState, AppendAppState, PopAppState] {
	...
	// Проверяем, чтобы состояние всегда было корректным
	if (!appState) {
		console.warn("AppState is undefined. Resetting to default state.");
		setAppStateAtom(new AppState(MenuStateType.MENU));
	}
